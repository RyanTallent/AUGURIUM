...
/**
 * PolymarketScan supplies leader trade/position intel for US live copy.
 * Retail US API keys cannot read exchange-wide trade reports (institutional Auth0 only).
 */
export function usePolymarketScanLeaderIntel(
  env: Record<string, string | undefined> = globalThis.process?.env ?? {},
): boolean {
  if (env.COPY_LEADER_INTEL_SOURCE === "polymarketscan") return true;
  if (env.COPY_LEADER_INTEL_SOURCE === "none") return false;
  return isUsOnlyArchitecture(env);
}

/** @deprecated Use usePolymarketScanLeaderIntel — kept for COPY_INTEL_SOURCE=polymarketscan */
export function usePolymarketScanIntel(
  env: Record<string, string | undefined> = globalThis.process?.env ?? {},
): boolean {
  return env.COPY_INTEL_SOURCE === "polymarketscan" || usePolymarketScanLeaderIntel(env);
}
