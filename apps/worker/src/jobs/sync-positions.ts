...
export async function syncPositionsForTrader(trader: {
  id: string;
  address: string;
}): Promise<number> {
  ...
  for (const pos of positions) {
    const { marketId } = await ensureMarketForPosition(pos);
...
export async function syncPositionsFromApi(): Promise<number> {
  ...
  try {
    ...
    for (const trader of traders) {
      synced += await syncPositionsForTrader(trader);
    }

    const metadata: Prisma.InputJsonValue = { positionsSynced: synced };
    ...
    console.log(`[sync-positions] synced=${synced}`);
    return synced;
  } catch (err) {
